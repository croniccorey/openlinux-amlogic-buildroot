...
vformat_t video_type_convert(enum CodecID id)
{
	...
	switch (id)
    {
    	...
    	default:
    	    format = -1;
            log_print("video_type_convert failed:video codec_id=%d\n", id);
    } 
    log_print("[video_type_convert]video codec_id=%d format=%d\n", id, format);
    return format;
}
...
stream_type stream_type_convert(pstream_type type, char vflag, char aflag)
{
	switch(type)
	{
		case STREAM_TS:
...
{		
	...
	{   
		...
		{                
			...
			{/*if the return is EAGAIN,we need to try more times*/
			    log_error("av_read_frame return (%d)\n",ret);
        ...
        {            
            ...
            //printf("av_read_frame return (%d) pkt->avpkt=%p pkt->avpkt->data=%p\n",ret,pkt->avpkt,pkt->avpkt->data);
	...
	{        
		...
		{            
            ...
            else
            {
                ...
                pkt->data_size-=len;
                player_thread_wait(para,RW_WAIT_TIME);   
                //log_print("[%s:%d]eagain---data_size=%d!\n",__FUNCTION__, __LINE__,pkt->data_size);             
                return PLAYER_SUCCESS;
            }
		}	
		else
	...
	if(para->stream_type == STREAM_ES)
	{
		if (INT64_0 != pkt->avpkt->pts)
	    {
	        pts = pkt->avpkt->pts * duration;
	    	 if(pts < para->vstream_info.start_time)
			 	pts = pts * last_duration;  

	    	if(codec_checkin_pts(pkt->codec, pts) != 0)
			{
				log_print("ERROR: check in pts error!\n");
		...
		{	
			...
			if(codec_checkin_pts(pkt->codec, pts)!=0)
			{
				log_print("ERROR: check in dts error!\n");
				return PLAYER_PTS_ERROR;
			}    
            if (pkt->type == CODEC_AUDIO)        
            {                
...
int set_header_info(play_para_t *para, am_packet_t *pkt)
{
	...
	if(pkt->avpkt_newflag)
    {
        ...
        if(pkt->type == CODEC_VIDEO)
        {
            ...
            {           
                ret = h264_update_frame_header(pkt->data, pkt->data_size);
                if(ret != PLAYER_SUCCESS)
                    return ret;
            }            
    		...
    		else if (para->vstream_info.video_format == VFORMAT_VC1)
    		{
    			if ( para->vstream_info.video_codec_type == VIDEO_DEC_FORMAT_WMV3 )
    			{
    	        	...
    	        	pkt->hdr->data[4] = 0;
    	        	pkt->hdr->data[5] = (pkt->data_size >> 16) & 0xff;
    	        	pkt->hdr->data[6] = 0x88;
    	        	pkt->hdr->data[7] = (pkt->data_size >> 8) & 0xff;
    	        	pkt->hdr->data[8] = pkt->data_size & 0xff;
    	        	pkt->hdr->data[9] = 0x88;
