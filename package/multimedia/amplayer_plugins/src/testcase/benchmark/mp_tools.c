...
int _ext_code_map_to_msg_dump(MP_ExtMsgCode* ext)
{
    ...
    else if(ext->type == MP_SERV_PPS)
    {

    }
    else if(ext->type == MP_SERV_XUNLEI)
    {
...
int _media_info_dump(MP_AVMediaFileInfo* minfo)
{
    ...
    if(minfo->atrack_counts > 0)
    {
        if(minfo->has_tag)
        {
            ...
            {
                printf("======||track title:%s",minfo->taginfo->title);   
                printf("\n======||track album:%s",minfo->taginfo->album); 
                printf("\n======||track author:%s\n",minfo->taginfo->author);  
                free(minfo->taginfo);
            }
...
int inform_msg_notify(void * obj,void *msg,int len)
{
    MP_ResponseMsg msgt;
    ...
    {        
        if(type == MP_RESP_STATE_CHANGED)
        {
              ...
              case MP_STATE_STOPED:
                  ...
                  MP_ClosePlayerInstance(msgt.mediaID);
                  break;
              ...
              case MP_STATE_CONNECTING:
                  printf("====== current status:%s\n","player connecting server");
                  break;
              case MP_STATE_CONNECTDONE:
                  printf("====== current status:%s\n","player  connect done");
                  break;
             ...
             default:
              ...
              if(NULL != ((MP_StateChangedRespBody*)msgt.auxDat)->stateEx&& msgt.auxDatLen > sizeof(MP_StateChangedRespBody) -4)
              {
...
int mp_run_tool()
{
    ...
    pGetMsgCallback foo = &inform_msg_notify;    

    register_get_msg_func(NULL,foo);
    ...
    while (!tmpneedexit) {     
        ...
        case EMU_STEP_PLAY:               
            do	{
	              ...
	              {	
		              ...
		              //EMU_INPUT("Please Input start time","0",tmpcommand,sizeof(tmpcommand));
		              //context.st = atoi(tmpcommand);
		              //EMU_INPUT("Please Input duration time","0",tmpcommand,sizeof(tmpcommand));
		              //context.st = atoi(tmpcommand);
		              ...
		              ret = MP_play_file(context.url,context.isStart,context.isLoop,context.st,context.kp_duration,context.plmode);
		              printf("=======||get mplayer mediaID:%d||======\n",ret);
        ...
        case EMU_STEP_MUTE:
            do {
                ...
                MP_mute(context.media_id,context.isMute);
                tmpstep = EMU_STEP_MENU;
            } while (0);
            break;                 
         case EMU_STEP_FF:
         ...
         case EMU_STEP_SETVOL:
                ...
                MP_SetVolume(media_id,context.vol);
                tmpstep = EMU_STEP_MENU;
            } while (0);
            break;         	  
         case EMU_STEP_GETVOL:
         	  do {
                ...
                if(context.isAsync == 1)
                    MP_GetVolume(media_id);
                else
                {
                    MP_GetVolumeSync(media_id,&ret);
         ...
         case EMU_STEP_QUITPLAYER:
           do {              
               MP_TerminateAmplayerProcess(media_id);
               tmpstep = EMU_STEP_MENU;
            } while (0);	
         	break; 
         ...
         case EMU_STEP_CLOSEMEDIAINST:
            do {             
              EMU_INPUT("Please Input media_id[default:11]","11",tmpcommand,sizeof(tmpcommand));
              context.media_id = atoi(tmpcommand); 
              MP_ClosePlayerInstance(context.media_id); 
              tmpstep = EMU_STEP_MENU;
            } while (0); 
