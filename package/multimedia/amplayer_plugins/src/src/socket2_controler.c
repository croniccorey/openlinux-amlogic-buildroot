            ...
            switch(cmdtype)
            {
                ...
                case MP_CMD_MUTE:                    
                    if(((MP_MuteMsgBody*)cmd.auxDat)->isMute)
                    {
                        ...
                        if(((MP_MuteMsgBody*)cmd.auxDat)->mediaID >= PID_RANDOM_NUMBER)
                            playercmd->pid = ((MP_MuteMsgBody*)cmd.auxDat)->mediaID - PID_RANDOM_NUMBER;
                        else
                            playercmd->pid = 0;
                ...
                case MP_CMD_SETTONE:
                    playercmd->cid = ((cmdtype&0x000000ff)<<8)+((cmd.head.client_id&0x0000ffff )<<16);
                    if((((MP_SetToneMsgBody*)cmd.auxDat)->mediaID) >= PID_RANDOM_NUMBER)
                        playercmd->pid =  (((MP_SetToneMsgBody*)cmd.auxDat)->mediaID) - PID_RANDOM_NUMBER;
                    else
                        playercmd->pid = 0;
...
{    
    ...
    if(body.stateEx&&(resp.auxDatLen > 4))
        free(body.stateEx);
    ...
    if(NULL == pData)
    {
        switch(type)
        {
            ...
            case CMD_PLAY_START:
                ...
                resp.mediaID = pid+ PID_RANDOM_NUMBER; 
                log_info("mplayer instance id:%d\n",resp.head.req_seq_no);
                resp.auxDat = NULL;
                resp.auxDatLen = 0;
                break;
            default:
                log_err("%s[%d] impossible case\n",__FUNCTION__,__LINE__);
                break;
        }
    }
    else
           ...
           case  CMD_LIST_PID:           
                {
                    ...
                    resp.auxDat= &pids;
                    resp.mediaID = pids.media_id[0] + PID_RANDOM_NUMBER;
           ...
           case CMD_GET_MEDIA_INFO:
                ...
                if(ctype == MP_CMD_GETMEDIAINFO)
                {
                    ...
                    if(NULL != _av_info->audio_info[0]&&NULL != _av_info->audio_info[0]->audio_tag)
                    {
                        ...
                        strncpy(taginfo.title,_av_info->audio_info[0]->audio_tag->title,TITLE_LEN_MAX);
                        strncpy(taginfo.album,_av_info->audio_info[0]->audio_tag->album,ALBUM_LEN_MAX);
                        strncpy(taginfo.author,_av_info->audio_info[0]->audio_tag->author,AUTHOR_LEN_MAX);
                        avbody.taginfo = &taginfo;                    
                        resp.auxDatLen += sizeof(MP_AudioTagInfo);
